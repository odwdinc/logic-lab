...
5. Click CLK high/low → Q never changes (STORE is off)`,
      build(cid) {
        ...
        for (let i = 0; i < 4; i++) {
          const bit = 3 - i;   // D3, D2, D1, D0 left to right
          const x = 240 + i * 300;
          const gD = addNode(cid, 'INPUT',   x,  60, `D${bit}`); gD._value = 0;
          const gR = addNode(cid, rDef.id,   x, 160, '');
          const gQ = addNode(cid, 'OUTPUT',  x, 340, `Q${bit}`);
          addWire(cid, gD.id,    'out',  gR.id, rData);
          addWire(cid, gClk.id,  'out',  gR.id, rClk);
          addWire(cid, gStore.id,'out',  gR.id, rStore);
          addWire(cid, gR.id,    rQ,     gQ.id, 'a');
        }
      },
    },
    {
      ...
      saveBlock: 'REG_4BIT',
      test: {
        inputs:  ['D3', 'D2', 'D1', 'D0', 'CLK', 'STORE'],
        outputs: ['Q3', 'Q2', 'Q1', 'Q0'],
        rows: [
          // Store 0101 (5): STORE=1, CLK=1 → Q=0101
          { in: [0, 1, 0, 1,  1, 1], out: [0, 1, 0, 1] },
          // Change data to 1010, STORE=0, CLK=1 → Q still 0101
          { in: [1, 0, 1, 0,  1, 0], out: [0, 1, 0, 1] },
          // CLK=0 → Q still holds
          { in: [1, 0, 1, 0,  0, 0], out: [0, 1, 0, 1] },
          // STORE=1, CLK=1 → Q captures 1010
          { in: [1, 0, 1, 0,  1, 1], out: [1, 0, 1, 0] },
        ],
      },
    },
    {
      title: 'Key Insight',
      text: `You have now built the same controlled register the script demonstrates at the end of Episode 2. When STORE is enabled and the clock ticks, exactly one value is captured — no racing, no glitching.
...
From here, the next step is Random Access Memory — an array of thousands of these registers that can be read from or written to by address. That is Episode 3.`,
    },
  ],
});
